...
public class MapCollectiveContainerLauncherImpl
  ...
  static final Log LOG =
    LogFactory
      .getLog(MapCollectiveContainerLauncherImpl.class);
   ...
   * nodemanager.
   */
  class MapCollectiveEventProcessor extends
    EventProcessor {
  ...
  private void editMapCollectiveLaunchEvent(
    ...
    try {
      ...
      // java opts
      String mcJavaOptsStr =
        task.getConf().get(
          MAP_COLLECTIVE_JAVA_OPTS, "");
      mcJavaOptsStr =
        mcJavaOptsStr.replace("@taskid@",
          attemptID.toString()).trim();
      ...
      if (!mcJavaOptsStr.equals("")) {
        ...
        // last one
        for (int i = 1; i < javaOpts.size(); i++) {
          newCommands =
            newCommands.replace(javaOpts.get(i),
              "");
        }
        // Replace the first one (0) to new java
        // opt string
        newCommands =
          newCommands.replace(javaOpts.get(0),
            mcJavaOptsStr);
      }
