...
public class MapCollectiveContainerAllocator
  ...
  static final Log LOG =
    LogFactory
      .getLog(MapCollectiveContainerAllocator.class);
   ...
   */
  private void
    getAllNodesInfo(Configuration conf)
    ...
    LOG
      .info("Get all nodes inforamtion from resource manager, use "
  ...
  protected synchronized void handleEvent(
    ...
    if (event.getType() == ContainerAllocator.EventType.CONTAINER_REQ) {
      ...
      if (event.getAttemptID().getTaskId()
        ...
        if (memory == 0) {
          memory =
            Integer.parseInt(getConfig().get(
              MAP_COLLECTIVE_MEMORY, "0"));
        }
