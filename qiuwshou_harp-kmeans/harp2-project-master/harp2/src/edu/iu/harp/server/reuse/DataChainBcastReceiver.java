...
public class DataChainBcastReceiver extends
  ...
  private Data receiveData(final InputStream in)
    ...
    if (sourceID != nextID) {
      nextConn =
        IOUtils.getConnection(nextHost, nextPort);
      if (nextConn == null) {
        getResourcePool().releaseBytes(opBytes);
    ...
    if (nextConn != null) {
      // nextConn.close();
      ConnectionPool.releaseConnection(nextConn);
    }
  ...
  private void receiveBytes(InputStream in,
    ...
    if (out != null) {
      int start = 0;
      int rest = size;
      while (rest > Constants.SEND_BYTE_UNIT) {
        int len =
          in.read(bytes, start,
      ...
      while (rest > 0) {
        int len = in.read(bytes, start, rest);
        if (len > 0) {
          out.write(bytes, start, len);
          out.flush();
        }
