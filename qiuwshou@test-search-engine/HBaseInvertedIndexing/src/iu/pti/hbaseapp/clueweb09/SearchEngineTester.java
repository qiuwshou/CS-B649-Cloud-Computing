...
public class SearchEngineTester {
	...
	public static void searchKeyword(String keyword) throws Exception {
        ...
        for (KeyValue kv : indexRow.list()) {
			...
			Get g = new Get(DocIdBytes);
			Result uriRow = dataTable.get(g);
			...
			Result prRow = prTable.get(g);
			byte[] prBytes = prRow.getFamilyMap(Constants.CF_PAGERANK_BYTES).firstEntry().getKey();
		    pageRank = Bytes.toFloat(prBytes);}
			catch (Exception e){
			  e.printStackTrace();
			}
