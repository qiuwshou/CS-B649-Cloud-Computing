...
/**
 * Allows sending events in multiple threads. So
 * this will be non-blocking io,user don't know if
 * sending is failed.
 * 
 * @author zhangbj
 *
 */
public class AsyncClient extends RecursiveAction {
  ...
  private class SendTask extends RecursiveAction {
    ...
    private final LinkedList<Transferable> eventList;
    private final String contextName;
    private final int sourceID;
    private final int destID;

    SendTask(LinkedList<Transferable> events,
      String name, int source, int dest) {
      eventList = events;
      contextName = name;
      sourceID = source;
      destID = dest;
    ...
    private void send() {
      ...
      // queue as events.
      if (eventList.size() == 1) {
        Transferable trans = eventList.getFirst();
        if (destID == ALL_WORKERS) {
        ...
        Data data =
          new Data(DataType.TRANS_LIST,
            contextName, sourceID, eventList);
        if (destID == ALL_WORKERS) {
          ...
          // for (Transferable trans : eventList)
          // {
          // eventQueue.addEvent(new Event(
          // EventType.COLLECTIVE_EVENT,
          // contextName, sourceID, trans));
    ...
    while (true) {
      ...
      if (obj instanceof QueueObj) {
        ...
        if (!eventList.isEmpty()) {
          SendTask sendTask =
            new SendTask(eventList,
              queueObj.getContextName(), selfID,
              queueObj.getDestID());
          sendTask.fork();
        }
      } else {
